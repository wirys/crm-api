...
import { registerJobType, recoverPendingJobs } from "../lib/job-queue";
...
export function registerJobs() {
    ...
    registerJobType(JOB_TABELA_PRECO_PUBLICAR, {
        handler: async (idReferencia, _payload, reportProgress) => {
            try {
                const resultado = await publicarTabelaPreco(idReferencia, reportProgress);
                return resultado;
            } catch (err) {
                if (idReferencia) {
                    await prisma.tbTabelaPreco.update({

 ...
 *  4. Todo o processamento roda dentro de uma única transaction: qualquer falha
 *     no meio desfaz tudo (a procedure antiga não tinha transaction própria e
 *     dependia inteiramente do chamador).
 *
...
export async function publicarTabelaPreco(idTabelaPreco: number | null) {
    if (!idTabelaPreco) {
    ...
    const composicoesParaPublicar = rows
    ...
    await prisma.$transaction(async (tx) => {
        ...
        if (idsComposicaoTocadas.length > 0) {
            ...
            if (vinculos.length > 0) {
                await tx.cRM_Produto_ComposicaoMaterial.createMany({ data: vinculos });
                resultado.vinculosRecriados += vinculos.length;
            }
        }
